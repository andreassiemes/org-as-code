...
def _cmd_serve(args) -> int:
    ...
    try:
        serve(args.path, host=args.host, port=args.port, key=args.key,
              watch=args.watch, ceiling=args.ceiling)
    except LoadError as exc:
        print(f"✗ {exc}")
        return 1
    return 0
...
def main(argv=None) -> int:
    ...
    s.add_argument("--ceiling", default="internal",
                   choices=["public", "internal", "restricted"],
                   help="highest tier this endpoint may serve (default: internal). "
                        "Entities above it are served as redacted cards. "
                        "`confidential` is not a choice: that tier is never served.")
